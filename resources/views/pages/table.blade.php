            ...
            <div class="flex justifiy-between gap-1 my-4 px-4 border-t border-gray-100 dark:border-gray-700 pt-4">
                <div class="flex justifiy-start w-full gap-2">
                    @if((bool)config('filament-plugins.allow_generator'))
                        <x-filament::icon-button tooltip="{{ trans('filament-plugins::messages.plugins.generate') }}" tag="a" href="{{route('filament.'.filament()->getCurrentPanel()->getId().'.resources.tables.index', ['module'=>$item->module_name])}}">
                            <x-slot name="icon">
                ...
                <div class="w-full flex justify-end gap-4">
                    ...
                    @endif
                    @if($item->docs)
                        <x-filament::icon-button tooltip="trans('filament-plugins::messages.plugins.docs'))" href="{{$item->docs}}" target="_blank" tag="a">
                            <x-slot name="icon">
